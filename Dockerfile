...
RUN apk --no-cache --allow-untrusted add /mariadb-apks/mariadb-common.apk /mariadb-apks/mariadb.apk \
 ...
 && tar -cvp -f /installed_files.tar $(apk manifest mariadb mariadb-common ncurses-libs libgcc xz-libs libaio pcre libstdc++ libressl2.7-libcrypto libressl2.7-libssl | awk -F "  " '{print $2;}') \
