...
ARG APKS="mariadb mariadb-client mariadb-server-utils libressl2.7-libssl libressl2.7-libcrypto libressl2.7-libtls ssl_client alpine-baselayout musl"
...
RUN echo /mariadb-apks >> /etc/apk/repositories \
 ...
 && sed -i '/libressl2.7-libtls/d' /pre_apks.list \
 && sed -i '/ssl_client/d' /pre_apks.list \
 && sed -i '/alpine-baselayout/d' /pre_apks.list \
 && sed -i '/musl/d' /pre_apks.list \
